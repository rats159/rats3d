...
public final class MathHelper {
   ...
   public static Matrix4f createTransformationMatrix(Vector3f translation, Vector3f rotation, float scale){
      ...
      mat.rotate((float)Math.toRadians(rotation.x),POSITIVE_X);
      mat.rotate((float)Math.toRadians(rotation.y),POSITIVE_Y);
      mat.rotate((float)Math.toRadians(rotation.z),POSITIVE_Z);
      mat.scale(scale);
      return mat;
   }
   ...
   public static Matrix4f createViewMatrix(Camera camera){
      Matrix4f mat = new Matrix4f();
      mat.identity();
      mat.rotate((float)Math.toRadians(camera.getPitch()), POSITIVE_X);
      mat.rotate((float)Math.toRadians(camera.getYaw()), POSITIVE_Y);
      Vector3f pos = camera.getPosition();
      mat.translate(new Vector3f(pos).negate());
      return mat;
   }
}
